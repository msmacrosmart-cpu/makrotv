...
export default function ServersPage() {
  ...
  async function del(id: string) {
    if (!confirm("Excluir servidor? Clientes vinculados ficarão sem DNS até serem reatribuídos.")) return;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-wrap gap-4 items-start justify-between">
        <div>
          <h1 className="text-2xl font-black flex items-center gap-3"><Server className="w-7 h-7 text-red-500" /> Servidores DNS</h1>
          <p className="text-white/50 text-sm mt-1">Cadastre e altere os endereços dos servidores sem recompilar o APK. Cada cliente pode estar vinculado a um servidor.</p>
        </div>
        <button onClick={openCreate} className="px-5 py-3 rounded-full bg-red-600 hover:bg-red-700 font-semibold flex items-center gap-2"><Plus className="w-5 h-5" /> Novo Servidor</button>
      </div>
      ...
      {servers.length === 0 && <div className="text-center py-16 text-white/40 bg-[#1a1a1a] border border-white/10 rounded-2xl">Nenhum servidor cadastrado. Crie o primeiro para o app voltar a funcionar.</div>}
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur flex items-center justify-center p-4">
          <div className="w-full max-w-md bg-[#1a1a1a] border border-white/10 rounded-3xl p-6">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-black">{editing ? "Editar Servidor" : "Novo Servidor"}</h2>
            ...
            <form onSubmit={submit} className="space-y-4">
              <div>
                ...
                <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="Ex: Servidor Principal" className="mt-2 w-full px-4 py-3 rounded-xl bg-[#262626] border border-white/10 focus:outline-none" />
              </div>
              <div>
